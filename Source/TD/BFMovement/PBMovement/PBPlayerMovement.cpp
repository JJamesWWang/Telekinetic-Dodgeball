...
DECLARE_CYCLE_STAT(
    TEXT("Char PhysFalling"), STAT_CharPhysFalling, STATGROUP_Character);

// MAGIC NUMBERS
const float MAX_STEP_SIDE_Z =
    0.08f; // maximum z value for the normal on the vertical side of steps
const float VERTICAL_SLOPE_NORMAL_Z =
    0.001f; // Slope is vertical if Abs(Normal.Z) <= this threshold. Accounts
// for precision problems that sometimes angle normals slightly
// off horizontal for vertical surface.
...
UPBPlayerMovement::UPBPlayerMovement()
{
    ...
    // Reasonable values polled from NASA
    // (https://msis.jsc.nasa.gov/sections/section04.htm#Figure%204.9.3-6) and
...
void UPBPlayerMovement::TickComponent(float DeltaTime, enum ELevelTick TickType,
    FActorComponentTickFunction* ThisTickFunction)
{
    bAppliedFriction = false;

    // TODO(mastercoms): HACK: double friction in order to account for
    // insufficient braking on substepping
    ...
    if ((bShowPos || CVarShowPos->GetInt() != 0) && CharacterOwner)
    {
        ...
        GEngine->AddOnScreenDebugMessage(2, 1.0f, FColor::Green,
            FString::Printf(TEXT("ang: %f %f %f"),
                CharacterOwner->GetControlRotation().Yaw,
                CharacterOwner->GetControlRotation().Pitch,
                CharacterOwner->GetControlRotation().Roll));
        GEngine->AddOnScreenDebugMessage(3, 1.0f, FColor::Green,
            FString::Printf(TEXT("vel: %f"),
                FMath::Sqrt(
		   ...
		   (Iterations < MaxSimulationIterations))
	{
		...
		if (!HasAnimRootMotion() && !CurrentRootMotion.HasOverrideVelocity())
		{
			...
			if (bHasAirControl)
			{
				...
				TGuardValue<FVector> RestoreAcceleration(
					Acceleration, FVector::ZeroVector);
				TGuardValue<FVector> RestoreVelocity(Velocity, Velocity);
				Velocity.Z = 0.f;
				CalcVelocity(timeTick, FallingLateralFriction, false, MaxDecel);
				VelocityNoAirControl =
					FVector(Velocity.X, Velocity.Y, OldVelocity.Z);
			}

			// Compute Velocity
			{
				// Acceleration = FallAcceleration for CalcVelocity(), but we
				// restore it after using it.
				TGuardValue<FVector> RestoreAcceleration(
			...
			// Just copy Velocity to VelocityNoAirControl if they are the same
			// (ie no acceleration).
			if (!bHasAirControl)
			{
		...
		{
			// Consume some of the force time. Only the remaining time (if any)
			// is affected by gravity when bApplyGravityWhileJumping=false.
			const float JumpForceTime =
				FMath::Min(CharacterOwner->JumpForceTimeRemaining, timeTick);
			GravityTime = bApplyGravityWhileJumping
							  ? timeTick
							  : FMath::Max(0.0f, timeTick - JumpForceTime);
		...
		VelocityNoAirControl =
			bHasAirControl
				? NewFallVelocity(VelocityNoAirControl, Gravity, GravityTime)
				: Velocity;
		const FVector AirControlAccel =
			(Velocity - VelocityNoAirControl) / timeTick;
		...
		if (IsSwimming())	 // just entered water
		{
			...
			StartSwimming(
				OldLocation, OldVelocity, timeTick, remainingTime, Iterations);
			return;
		}
		else if (Hit.bBlockingHit)
		{
			if (IsValidLandingSpot(
					UpdatedComponent->GetComponentLocation(), Hit))
			{
				remainingTime += subTimeTickRemaining;
			...
			{
				// Compute impact deflection based on final velocity, not
				// integration step. This allows us to compute a new velocity
				// from the deflected vector, and ensures the full gravity
				// effect is included in the slide result.
				Adjusted = Velocity * timeTick;

				// See if we can convert a normally invalid landing spot (based
				// on the hit result) to a usable one.
				if (!Hit.bStartPenetrating)
				{
					// hit a barrier, try to step up
					const FVector GravDir(0.f, 0.f, -1.f);
					bool bSteppedUp = StepUp(GravDir,
						FVector(Velocity.X, Velocity.Y, 0.f) *
					...
					if (bSteppedUp)
					{
						...
						if (Velocity.Z > 0.f)
						{
							...
							const float RemainingHeight =
								Velocity.Z * Velocity.Z / DoubleGravity -
								(UpdatedComponent->GetComponentLocation().Z -
									OldLocation.Z);
							if (RemainingHeight > 0.f)
							{
								Velocity.Z = FMath::Sqrt(
									RemainingHeight * DoubleGravity);
								Landed = false;
							}
						}
					...
					if (ShouldCheckForValidLandingSpot(timeTick, Adjusted, Hit))
					{
						...
						{
							remainingTime += subTimeTickRemaining;
							ProcessLanded(FloorResult.HitResult, remainingTime,
								Iterations);
							return;
						}
					}
				...
				// We moved to the impact point using air control, but may want
				// to deflect from there based on a limited air control
				// acceleration.
				if (bHasAirControl)
				{
					const bool bCheckLandingSpot =
						false;	  // we already checked above.
					const FVector AirControlDeltaV =
						LimitAirControl(LastMoveTimeSlice, AirControlAccel, Hit,
							bCheckLandingSpot) *
						LastMoveTimeSlice;
					Adjusted = (VelocityNoAirControl + AirControlDeltaV) *
							   LastMoveTimeSlice;
				}

				const FVector OldHitNormal = Hit.Normal;
				const FVector OldHitImpactNormal = Hit.ImpactNormal;
				FVector Delta = ComputeSlideVector(
					Adjusted, 1.f - Hit.Time, OldHitNormal, Hit);

				// Compute velocity after deflection (only gravity component for
				// RootMotion)
					...
					(Delta | Adjusted) > 0.f)
				{
					...
					if (Hit.bBlockingHit)
					{
						...
						// Act as if there was no air control on the last move
						// when computing new deflection.
						if (bHasAirControl &&
							Hit.Normal.Z > VERTICAL_SLOPE_NORMAL_Z)
						{
							const FVector LastMoveNoAirControl =
								VelocityNoAirControl * LastMoveTimeSlice;
							Delta = ComputeSlideVector(
								LastMoveNoAirControl, 1.f, OldHitNormal, Hit);
						}

						FVector PreTwoWallDelta = Delta;
						TwoWallAdjust(Delta, Hit, OldHitNormal);

						// Limit air control, but allow a slide along the second
						// wall.
						if (bHasAirControl)
						{
							const bool bCheckLandingSpot =
							...
							const FVector AirControlDeltaV =
								LimitAirControl(subTimeTickRemaining,
									AirControlAccel, Hit, bCheckLandingSpot) *
								subTimeTickRemaining;

							// Only allow if not back in to first wall
							if (FVector::DotProduct(
									AirControlDeltaV, OldHitNormal) > 0.f)
							{
								Delta +=
									(AirControlDeltaV * subTimeTickRemaining);
							}
						}

						// Compute velocity after deflection (only gravity
						// component for RootMotion)
						if (subTimeTickRemaining > KINDA_SMALL_NUMBER &&
							!bJustTeleported)
						{
							const FVector NewVelocity =
								(Delta / subTimeTickRemaining);
							Velocity =
								HasAnimRootMotion() &&
										!CurrentRootMotion.HasOverrideVelocity()
									? FVector(
										  Velocity.X, Velocity.Y, NewVelocity.Z)
									: NewVelocity;
						}

						// bDitch=true means that pawn is straddling two slopes,
						// neither of which he can stand on
						bool bDitch =
							((OldHitImpactNormal.Z > 0.f) &&
								(Hit.ImpactNormal.Z > 0.f) &&
								(FMath::Abs(Delta.Z) <= KINDA_SMALL_NUMBER) &&
								((Hit.ImpactNormal | OldHitImpactNormal) <
									0.f));
						SafeMoveUpdatedComponent(
							Delta, PawnRotation, true, Hit);
						if (Hit.Time == 0.f)
						{
							// if we are stuck then try to side step
							FVector SideDelta =
								(OldHitNormal + Hit.ImpactNormal)
									.GetSafeNormal2D();
							if (SideDelta.IsNearlyZero())
							{
								SideDelta =
									FVector(OldHitNormal.Y, -OldHitNormal.X, 0)
										.GetSafeNormal();
							}
							SafeMoveUpdatedComponent(
								SideDelta, PawnRotation, true, Hit);
						}
						...
						if (bDitch ||
							IsValidLandingSpot(
								UpdatedComponent->GetComponentLocation(),
								Hit) ||
							FMath::IsNearlyZero(Hit.Time))
						{
							remainingTime = 0.f;
							ProcessLanded(Hit, remainingTime, Iterations);
							return;
						}
						else if (GetPerchRadiusThreshold() > 0.f &&
								 FMath::IsNearlyEqual(Hit.Time, 1.0f) &&
								 OldHitImpactNormal.Z >= GetWalkableFloorZ())
						{
							// We might be in a virtual 'ditch' within our perch
							// radius. This is rare.
							const FVector PawnLocation =
								UpdatedComponent->GetComponentLocation();
							const float ZMovedDist =
								FMath::Abs(PawnLocation.Z - OldLocation.Z);
							const float MovedDist2DSq =
								(PawnLocation - OldLocation).SizeSquared2D();
							if (ZMovedDist <= 0.2f * timeTick &&
								MovedDist2DSq <= 4.f * timeTick)
							{
								Velocity.X += 0.25f * GetMaxSpeed() *
											  (FMath::FRand() - 0.5f);
								Velocity.Y += 0.25f * GetMaxSpeed() *
											  (FMath::FRand() - 0.5f);
								Velocity.Z = FMath::Max<float>(
									JumpZVelocity * 0.25f, 1.f);
...
bool UPBPlayerMovement::CanStepUp(const FHitResult& Hit) const
{
	...
	if (IsFalling())
	{
		...
		FVector Start =
			GetCharacterOwner()->GetCapsuleComponent()->GetComponentLocation() -
			FVector(0.0f, 0.0f,
				GetCharacterOwner()
					->GetCapsuleComponent()
					->GetScaledCapsuleHalfHeight());
		float FloorSweepTraceDist =
			MaxStepHeight / 2.0f + MAX_FLOOR_DIST + KINDA_SMALL_NUMBER;
		FVector End = Start - FVector(0.0f, 0.0f, FloorSweepTraceDist);
		GetWorld()->LineTraceSingleByChannel(HitResult, Start, End,
			ECollisionChannel::ECC_WorldStatic,
			FCollisionQueryParams(
				FName(TEXT("FallingStepTrace")), true, GetCharacterOwner()));
		if (!HitResult.bBlockingHit)
		{
			return false;
		}
		if ((HitResult.ImpactNormal | FVector(0.0f, 0.0f, 1.0f)) <
			GetWalkableFloorZ())
		{
			return false;
		}
...
bool UPBPlayerMovement::StepUp(const FVector& GravDir, const FVector& Delta,
	const FHitResult& InHit, FStepDownResult* OutStepDownResult)
{
	...
	float StepTravelUpHeight =
		IsFalling() ? MaxStepHeight / 3.0f : MaxStepHeight;
	float StepTravelDownHeight = StepTravelUpHeight;
	const float StepSideZ =
		-1.f * FVector::DotProduct(InHit.ImpactNormal, GravDir);
	float PawnInitialFloorBaseZ = OldLocation.Z - PawnHalfHeight;
	float PawnFloorPointZ = PawnInitialFloorBaseZ;

	if (IsMovingOnGround() && CurrentFloor.IsWalkableFloor())
	{
		// Since we float a variable amount off the floor, we need to enforce
		// max step height off the actual point of impact with the floor.
		const float FloorDist =
			FMath::Max(0.f, CurrentFloor.GetDistanceToFloor());
		PawnInitialFloorBaseZ -= FloorDist;
		StepTravelUpHeight = FMath::Max(StepTravelUpHeight - FloorDist, 0.f);
		StepTravelDownHeight = (MaxStepHeight + MAX_FLOOR_DIST * 2.f);

		const bool bHitVerticalFace = !IsWithinEdgeTolerance(
			InHit.Location, InHit.ImpactPoint, PawnRadius);
		...
		{
			// Base floor point is the base of the capsule moved down by how far
			// we are hovering over the surface we are hitting.
			PawnFloorPointZ -= CurrentFloor.FloorDist;
		}
	}

	// Don't step up if the impact is below us, accounting for distance from
	// floor.
	...
	// Scope our movement updates, and do not apply them until all intermediate
	// moves are completed.
	FScopedMovementUpdate ScopedStepUpMovement(
	...
	MoveUpdatedComponent(
		-GravDir * StepTravelUpHeight, PawnRotation, true, &SweepUpHit);
	...
	if (Hit.bBlockingHit)
	{
		...
		// If we hit something above us and also something ahead of us, we
		// should notify about the upward hit as well. The forward hit will be
		// handled later (in the bSteppedOver case below). In the case of
		// hitting something above but not forward, we are not blocked from
		// moving so we don't need the notification.
		...
		const float ForwardSlideAmount =
		...
		// If both the forward hit and the deflection got us nowhere, there is
		// no point in this step up.
		if (FMath::IsNearlyZero(ForwardHitTime) &&
			FMath::IsNearlyZero(ForwardSlideAmount))
		{
			ScopedStepUpMovement.RevertMove();
			return false;
		}
	...
	// Step down
	MoveUpdatedComponent(GravDir * StepTravelDownHeight,
	...
	{
		// See if this step sequence would have allowed us to travel higher than
		// our max step height allows.
		const float DeltaZ = Hit.ImpactPoint.Z - PawnFloorPointZ;
		if (DeltaZ > MaxStepHeight)
		{
			// UE_LOG(LogCharacterMovement, VeryVerbose, TEXT("- Reject StepUp
			// (too high Height %.3f) up from floor base %f to %f"), DeltaZ,
		...
		if (!IsWalkable(Hit))
		{
			...
			if (bNormalTowardsMe)
			{
				// UE_LOG(LogCharacterMovement, VeryVerbose, TEXT("- Reject
				// StepUp (unwalkable normal %s opposed to movement)"),
				// *Hit.ImpactNormal.ToString());
				ScopedStepUpMovement.RevertMove();
				return false;
			}

			// Also reject if we would end up being higher than our starting
			// location by stepping down. It's fine to step down onto an
			// unwalkable normal below us, we will just slide off. Rejecting
			// those moves would prevent us from being able to walk off the
			// edge.
			if (Hit.Location.Z > OldLocation.Z)
			{
				// UE_LOG(LogCharacterMovement, VeryVerbose, TEXT("- Reject
				// StepUp (unwalkable normal %s above old position)"),
				// *Hit.ImpactNormal.ToString());
				ScopedStepUpMovement.RevertMove();
				return false;
			}
		...
		// Reject moves where the downward sweep hit something very close to the
		// edge of the capsule. This maintains consistency with FindFloor as
		// well.
		if (!IsWithinEdgeTolerance(Hit.Location, Hit.ImpactPoint, PawnRadius))
		{
			// UE_LOG(LogCharacterMovement, VeryVerbose, TEXT("- Reject StepUp
			// (outside edge tolerance)"));
			ScopedStepUpMovement.RevertMove();
			return false;
		}
		...
		{
			// UE_LOG(LogCharacterMovement, VeryVerbose, TEXT("- Reject StepUp
			// (up onto surface with !CanStepUp())"));
			ScopedStepUpMovement.RevertMove();
			return false;
		}

		// See if we can validate the floor as a result of this step down. In
		// almost all cases this should succeed, and we can avoid computing the
		// floor outside this method.
		if (OutStepDownResult != NULL)
		{
			FindFloor(UpdatedComponent->GetComponentLocation(),
				StepDownResult.FloorResult, false, &Hit);

			// Reject unwalkable normals if we end up higher than our initial
			// height. It's fine to walk down onto an unwalkable surface, don't
			// reject those moves.
			if (Hit.Location.Z > OldLocation.Z)
			{
				// We should reject the floor result if we are trying to step up
				// an actual step where we are not able to perch (this is rare).
				// In those cases we should instead abort the step up and try to
				// slide along the stair.
				if (!StepDownResult.FloorResult.bBlockingHit &&
					StepSideZ < MAX_STEP_SIDE_Z)
				{
					ScopedStepUpMovement.RevertMove();
					return false;
				}
	...
	// Don't recalculate velocity based on this height adjustment, if
...
void UPBPlayerMovement::TwoWallAdjust(
    FVector& OutDelta, const FHitResult& Hit, const FVector& OldHitNormal) const
{
    Super::Super::TwoWallAdjust(OutDelta, Hit, OldHitNormal);
}

float UPBPlayerMovement::SlideAlongSurface(const FVector& Delta, float Time,
    const FVector& Normal, FHitResult& Hit, bool bHandleImpact)
{
    return Super::Super::SlideAlongSurface(
        Delta, Time, Normal, Hit, bHandleImpact);
}

FVector UPBPlayerMovement::HandleSlopeBoosting(const FVector& SlideResult,
    const FVector& Delta, const float Time, const FVector& Normal,
    const FHitResult& Hit) const
{
    return SlideResult;
}

bool UPBPlayerMovement::ShouldCatchAir(
    const FFindFloorResult& OldFloor, const FFindFloorResult& NewFloor)
{
    ...
    float ZDiff =
        NewFloor.HitResult.ImpactNormal.Z - OldFloor.HitResult.ImpactNormal.Z;
...
void UPBPlayerMovement::OnMovementModeChanged(
    EMovementMode PreviousMovementMode, uint8 PreviousCustomMode)
{
    ...
    {
        // Hit =
        // UPBUtil::TraceLineFullCharacter(CharacterOwner->GetCapsuleComponent(),
        // GetWorld(), CharacterOwner);
        FCollisionQueryParams TraceParams(
            FName(TEXT("RV_Trace")), true, CharacterOwner);
        TraceParams.bTraceComplex =
            CharacterOwner->GetCapsuleComponent()->bTraceComplexOnMove;
        TraceParams.bReturnPhysicalMaterial = true;

        GetWorld()->SweepSingleByChannel(Hit,
            CharacterOwner->GetCapsuleComponent()->GetComponentLocation(),
            CharacterOwner->GetCapsuleComponent()->GetComponentLocation() -
            FVector(0.0f, 0.0f,
    ...
    if (!MoveSound)
    {
        if (!PBCharacter->GetMoveStepSound(TEnumAsByte<EPhysicalSurface>(
            EPhysicalSurface::SurfaceType_Default)))
        {
            return;
        }
        MoveSound = PBCharacter
                    ->GetMoveStepSound(TEnumAsByte<EPhysicalSurface>(
                        EPhysicalSurface::SurfaceType_Default))
                    ->GetDefaultObject();
    }
    ...
    if (MoveSound)
    {
        ...
        TArray<USoundCue*> MoveSoundCues =
            bJumped ? MoveSound->GetJumpSounds() : MoveSound->GetLandSounds();
        ...
        USoundCue* Sound =
            MoveSoundCues[FMath::RandRange(0, MoveSoundCues.Num() - 1)];
        ...
        UGameplayStatics::SpawnSoundAttached(
            Sound, GetCharacterOwner()->GetRootComponent());
    }
}
...
void UPBPlayerMovement::ApplyVelocityBraking(
    float DeltaTime, float Friction, float BrakingDeceleration)
{
    float Speed = Velocity.Size2D();
    if (Speed <= 0.1f || !HasValidData() || HasAnimRootMotion() ||
    ...
    const FVector RevAccel =
        Friction * BrakingDeceleration * Velocity.GetSafeNormal();
...
void UPBPlayerMovement::PlayMoveSound(float DeltaTime)
{
    ...
    // ladder
    bool bPlaySound = (bBrakingFrameTolerated || bOnLadder) &&
                      Speed >= RunSpeedThreshold * RunSpeedThreshold;
    ...
    if (bOnLadder)
    {
        ...
        if (!PBCharacter->GetMoveStepSound(
            TEnumAsByte<EPhysicalSurface>(EPhysicalSurface::SurfaceType1)))
        {
            return;
        }
        MoveSound = PBCharacter
    ...
    else
    {
        ...
        {
            GotSound =
                PBCharacter->GetMoveStepSound(Hit.PhysMaterial->SurfaceType);
        }
        if (GotSound)
        {
            MoveSound = GotSound->GetDefaultObject();
        ...
        MoveSoundVolume = bSprinting
                              ? MoveSound->GetSprintVolume()
                              : MoveSound->GetWalkVolume();
    ...
    if (MoveSound)
    {
        TArray<USoundCue*> MoveSoundCues =
            StepSide
                ? MoveSound->GetStepLeftSounds()
                : MoveSound->GetStepRightSounds();
		...
		FVector Start =
			...
			FVector(0.0f, 0.0f,
				GetCharacterOwner()
					...
					->GetScaledCapsuleHalfHeight());
		float FloorSweepTraceDist =
			MaxStepHeight + MAX_FLOOR_DIST + KINDA_SMALL_NUMBER;
		FVector End = Start - FVector(0.0f, 0.0f, FloorSweepTraceDist);
		GetWorld()->LineTraceSingleByChannel(HitResult, Start, End,
			UpdatedComponent->GetCollisionObjectType(),
			FCollisionQueryParams(
				FName(TEXT("SurfTrace")), true, GetCharacterOwner()));
		if (HitResult.bBlockingHit &&
			HitResult.ImpactNormal.Z < GetWalkableFloorZ())
		{
			FVector MovementVector(Velocity.X, Velocity.Y, Velocity.Z);
			// vf = vi + at
			MovementVector.Z += (GetCharacterOwner()
									 ->GetCapsuleComponent()
									 ->GetPhysicsLinearVelocity()
									 .Z +
								 GetGravityZ() * DeltaTime *
									 (1.0f - HitResult.ImpactNormal.Z));
...
void UPBPlayerMovement::CalcVelocity(
    float DeltaTime, float Friction, bool bFluid, float BrakingDeceleration)
{
    ...
    if (ApplyRequestedMove(DeltaTime, MaxAccel, MaxSpeed, Friction,
    ...
    if (bForceMaxAccel)
    {
        ...
        // In consideration order for direction: Acceleration, then Velocity,
        // then Pawn's rotation.
        ...
        {
            Acceleration =
                MaxAccel * (Velocity.SizeSquared() < SMALL_NUMBER
    ...
    // Path following above didn't care about the analog modifier, but we do for
    // everything else below, so get the fully modified value. Use max of
    // requested speed and max speed if we modified the speed in
    // ApplyRequestedMove above.
    const float MaxInputSpeed =
        FMath::Max(MaxSpeed * AnalogInputModifier, GetMinAnalogSpeed());
    ...
    // Apply friction
    // TODO: HACK: friction applied only once in substepping due to excessive
    // friction, but this is too little for low frame rates
    if (bIsGroundMove && !bAppliedFriction)
    {
        const float ActualBrakingFriction =
            (bUseSeparateBrakingFriction ? BrakingFriction : Friction) *
            SurfaceFriction;
        ApplyVelocityBraking(
            DeltaTime, ActualBrakingFriction, BrakingDeceleration);
    ...
    if (bCheatFlying)
    {
        ...
        else
        {
            ...
            auto NoClipAccelClamp = PBCharacter->IsSprinting()
                                        ? 2.0f * MaxAcceleration
                                        : MaxAcceleration;
            Velocity =
                (Dir * LookVec * PerpendicularAccel.Size2D() + TangentialAccel)
                .GetClampedToSize(NoClipAccelClamp, NoClipAccelClamp);
        }
    }
    else
    {
        ...
        if (!bZeroAcceleration)
        {
            ...
            const float Veer =
                Velocity.X * AccelDir.X + Velocity.Y * AccelDir.Y;
            // Get add speed with air speed cap
            const float AddSpeed =
                (bIsGroundMove
                     ? Acceleration
                     : Acceleration.GetClampedToMaxSize2D(AirSpeedCap))
                .Size2D() -
                Veer;
            if (AddSpeed > 0.0f)
            {
                ...
                Acceleration *=
                    AccelerationMultiplier * SurfaceFriction * DeltaTime;
        ...
        // Dynamic step height code for allowing sliding on a slope when at a
        // high speed
        if (SpeedSq <= MaxWalkSpeedCrouched * MaxWalkSpeedCrouched)
        {
            // If we're crouching or not sliding, just use max
            MaxStepHeight = GetClass()
                            ->GetDefaultObject<UPBPlayerMovement>()
                            ->MaxStepHeight;
            SetWalkableFloorZ(GetClass()
                              ->GetDefaultObject<UPBPlayerMovement>()
                              ->GetWalkableFloorZ());
        }
        else
        {
            ...
            float SpeedScale =
                (Speed - SpeedMultMin) / (SpeedMultMax - SpeedMultMin);
            ...
            if (!IsFalling())
            {
                ...
                SpeedMultiplier = FMath::Max(
                    (1.0f - SurfaceFriction) * SpeedMultiplier, 0.0f);
            }
            MaxStepHeight =
                FMath::Clamp(GetClass()
                             ->GetDefaultObject<UPBPlayerMovement>()
                             ->MaxStepHeight *
...
void UPBPlayerMovement::DoCrouchResize(
    float TargetTime, float DeltaTime, bool bClientSimulation)
{
    ...
    auto DefaultCharacter =
        CharacterOwner->GetClass()->GetDefaultObject<ACharacter>();

    if (bClientSimulation &&
        CharacterOwner->GetLocalRole() == ROLE_SimulatedProxy)
    {
        // restore collision size before crouching
        CharacterCapsule->SetCapsuleSize(
    ...
    float CurrentAlpha =
        1.0f -
        (CurrentUnscaledHalfHeight - CrouchedHalfHeight) / FullCrouchDiff;
    ...
    float TargetCrouchedHalfHeight =
        OldUnscaledHalfHeight - FullCrouchDiff * TargetAlpha;
    // Height is not allowed to be smaller than radius.
    float ClampedCrouchedHalfHeight =
        FMath::Max3(0.0f, OldUnscaledRadius, TargetCrouchedHalfHeight);
    CharacterCapsule->SetCapsuleSize(
        OldUnscaledRadius, ClampedCrouchedHalfHeight);
    ...
    if (!bClientSimulation)
    {
        if (bCrouchMaintainsBaseLocation)
        {
            ...
            UpdatedComponent->MoveComponent(
                FVector(0.0f, 0.0f, -ScaledHalfHeightAdjust),
        ...
        CharacterOwner->GetLocalRole() == ROLE_SimulatedProxy)
    {
            ...
            !FMath::IsNearlyZero(ClientData->MeshTranslationOffset.Z))
        {
            ClientData->MeshTranslationOffset -=
                FVector(0.0f, 0.0f, ScaledHalfHeightAdjust);
            ClientData->OriginalMeshTranslationOffset =
                ClientData->MeshTranslationOffset;
        }
    }
}
...
void UPBPlayerMovement::DoUnCrouchResize(
    float TargetTime, float DeltaTime, bool bClientSimulation)
{
    if (!HasValidData())
    {
    ...
    if (!bClientSimulation)
    {
        ...
        FCollisionQueryParams CapsuleParams(
            SCENE_QUERY_STAT(CrouchTrace), false, CharacterOwner);
        ...
        const FCollisionShape StandingCapsuleShape =
            GetPawnCapsuleCollisionShape(SHRINK_HeightCustom,
                -SweepInflation -
                ScaledHalfHeightAdjust); // Shrink by
        // negative amount,
        // so actually grow it.
        ...
        if (!bCrouchMaintainsBaseLocation)
        {
            ...
            bEncroached = MyWorld->OverlapBlockingTestByChannel(PawnLocation,
                FQuat::Identity, CollisionChannel, StandingCapsuleShape,
            ...
            if (bEncroached)
            {
                ...
                if (ScaledHalfHeightAdjust > 0.0f)
                {
                    ...
                    // from there.
                    float PawnRadius, PawnHalfHeight;
                    CharacterCapsule->GetScaledCapsuleSize(
                        PawnRadius, PawnHalfHeight);
                    ...
                    const bool bBlockingHit = MyWorld->SweepSingleByChannel(Hit,
                        PawnLocation, PawnLocation + Down, FQuat::Identity,
                        CollisionChannel, ShortCapsuleShape, CapsuleParams);
                    if (Hit.bStartPenetrating)
                    {
                        bEncroached = true;
                    }
                    else
                    {
                        ...
                        const float DistanceToBase =
                            (Hit.Time * TraceDist) +
                            ShortCapsuleShape.Capsule.HalfHeight;
                        const FVector NewLoc =
                            FVector(PawnLocation.X, PawnLocation.Y,
                                PawnLocation.Z - DistanceToBase +
                                StandingCapsuleShape.Capsule.HalfHeight +
                                SweepInflation + MIN_FLOOR_DIST / 2.0f);
                        bEncroached = MyWorld->OverlapBlockingTestByChannel(
                            NewLoc, FQuat::Identity, CollisionChannel,
        ...
        else
        {
            ...
            FVector StandingLocation =
                PawnLocation + FVector(0.0f, 0.0f,
                    StandingCapsuleShape.GetCapsuleHalfHeight() -
                    CurrentCrouchedHalfHeight);
            bEncroached = MyWorld->OverlapBlockingTestByChannel(
                StandingLocation, FQuat::Identity, CollisionChannel,
                StandingCapsuleShape, CapsuleParams, ResponseParam);
            ...
            if (bEncroached)
            {
                if (IsMovingOnGround())
                {
                    ...
                    if (CurrentFloor.bBlockingHit &&
                        CurrentFloor.FloorDist > MinFloorDist)
                    {
                        StandingLocation.Z -=
                            CurrentFloor.FloorDist - MinFloorDist;
                        bEncroached = MyWorld->OverlapBlockingTestByChannel(
            ...
            if (!bEncroached)
            {
                ...
                UpdatedComponent->MoveComponent(StandingLocation - PawnLocation,
                    UpdatedComponent->GetComponentQuat(), false, nullptr,
                    EMoveComponentFlags::MOVECOMP_NoFlags,
                    ETeleportType::TeleportPhysics);
                bForceNextFloorCheck = true;
            }
