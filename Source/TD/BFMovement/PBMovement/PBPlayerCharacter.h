...
UCLASS(config = Game)
class TD_API APBPlayerCharacter : public ACharacter
{
    GENERATED_BODY()
...
public:
    ...
    void OnMovementModeChanged(
        EMovementMode PrevMovementMode, uint8 PrevCustomMode) override;
    ...
    /** NOTE: Changed from private to protected to override
     * OnJumped_Implementation() to replicate properly. */
protected:
    ...
    UPROPERTY
    (EditDefaultsOnly, meta = (AllowPrivateAccess = "true"),
        Category = "PB Player|Sounds")
    TMap<TEnumAsByte<EPhysicalSurface>, TSubclassOf<UPBMoveStepSound>>
    MoveStepSounds;
...
#pragma region Mutators
    ...
    FORCEINLINE TSubclassOf<UPBMoveStepSound>* GetMoveStepSound(
