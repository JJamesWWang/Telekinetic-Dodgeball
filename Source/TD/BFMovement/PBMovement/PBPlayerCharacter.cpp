...
APBPlayerCharacter::APBPlayerCharacter(
    ...
    : Super(ObjectInitializer.SetDefaultSubobjectClass<UPBPlayerMovement>(
        ACharacter::CharacterMovementComponentName))
{
    ...
    // Set collision settings. We are the invisible player with no 3rd person
    // mesh.
...
void APBPlayerCharacter::BeginPlay()
{
    ...
    MaxJumpTime = -4.0f * GetCharacterMovement()->JumpZVelocity /
                  (3.0f * GetCharacterMovement()->GetGravityZ());
}

void APBPlayerCharacter::ClearJumpInput(float DeltaTime)
{
    // Don't clear jump input right away if we're auto hopping or noclipping
    // (holding to go up)
    if (bAutoBunnyhop || GetCharacterMovement()->bCheatFlying)
    {
        return;
    }
...
void APBPlayerCharacter::OnMovementModeChanged(
    EMovementMode PrevMovementMode, uint8 PrevCustomMode)
{
    if (!bPressedJump)
    {
        ResetJumpState();
    ...
    {
        // Record jump force start time for proxies. Allows us to expire the
        // jump even if not continually ticking down a timer.
    ...
    K2_OnMovementModeChanged(PrevMovementMode,
        GetCharacterMovement()->MovementMode, PrevCustomMode,
        GetCharacterMovement()->CustomMovementMode);
    MovementModeChangedDelegate.Broadcast(
        this, PrevMovementMode, PrevCustomMode);
}
...
void APBPlayerCharacter::StopJumping()
{
    ...
    {
        Cast<UPBPlayerMovement>(GetMovementComponent())
...
{
    bool bCanJump =
        GetCharacterMovement() && GetCharacterMovement()->IsJumpAllowed();
    ...
    if (bCanJump)
    {
        ...
        else
        {
            ...
            const bool bJumpKeyHeld =
                (bPressedJump && JumpKeyHoldTime < GetJumpMaxHoldTime());
        ...
        if (GetCharacterMovement()->IsMovingOnGround())
        {
            float FloorZ =
                ...
                GetCharacterMovement()->CurrentFloor.HitResult.ImpactNormal;
            float WalkableFloor = GetCharacterMovement()->GetWalkableFloorZ();
            bCanJump &= (FloorZ >= WalkableFloor ||
                         FMath::IsNearlyEqual(FloorZ, WalkableFloor));
        }
    }
...
void APBPlayerCharacter::ApplyDamageMomentum(float DamageTaken,
    FDamageEvent const& DamageEvent, APawn* PawnInstigator,
    AActor* DamageCauser)
{
    UDamageType const* const DmgTypeCDO =
    ...
    if (GetCharacterMovement())
    {
        ...
        float SizeFactor =
            (60.96f * 60.96f * 137.16f) /
            (FMath::Square(
                 GetCapsuleComponent()->GetScaledCapsuleRadius() * 2.0f) *
        ...
        if (!bMassIndependentImpulse &&
            GetCharacterMovement()->Mass > SMALL_NUMBER)
        {
            MassScaledImpulse =
                MassScaledImpulse / GetCharacterMovement()->Mass;
        }
